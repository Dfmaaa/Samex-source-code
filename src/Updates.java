...
public class Updates{
    ...
    static String[] fullups() throws MalformedURLException, Exception{
        if(System.getProperty("os.name").contains("Windows")){
          downloadFile(new URL("https://samexmain.github.io/full_win"),"winups");
      }
      else{
            downloadFile(new URL("https://samexmain.github.io/full_lin"),"linuxups");
      }
      if(System.getProperty("os.name").contains("Windows")){
          return getarr(new File("winups"));
      }
    ...
    public static void updmethod() throws Exception {
        ...
        if(see.equals(ver)){
            System.out.println("No updates!");
                ...
                System.out.println("If you feel like you've missed any updates, you can use the F option to update all files.");
                ...
                if(choice.equals("Y")){
                  ...
                  FileWriter wr=new FileWriter("samex_version.txt");
                    wr.write(see);
                    wr.close();
                ...
                if(choice.equals("F")){
                  ...
                  for(int x=0;x<full.length;x++){
                      String url=full[x].split(",")[0];
                      String fname=full[x].split(",")[1];
                      String loc=full[x].split(",")[2];
                      System.out.println("Deleting old " + fname + "...");
                      new File(loc).delete();
                         System.out.println("Downloading new " + fname + "...");
                           downloadFile(new URL(url),loc);
                           count++;
                           System.out.println(count + " files updated. (" + (percen(count,full.length)*100)+ "%)");
                  }
                 FileWriter wr=new FileWriter("samex_version.txt");
                   wr.write(see);
                   wr.close();
