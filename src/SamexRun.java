...
public class SamexRun {
	...
	static String encrypt(String data){
		int counter=ascii(data.charAt(0));
		String data2=data;
	...
	static boolean incorrectcmd(String command){
		if(command.equals("help")==false&command.equals("packages")==false&command.equals("update")==false&command.equals("restart")==false&command.equals("repair")==false&command.equals("Atlas")==false&command.equals("rock paper scissors")==false&command.equals("settings")==false&command.equals("showalpha")==false&command.equals("emdir")==false&command.equals("playfile")==false&command.equals("goto")==false&command.equals("getlines")==false&command.equals("remove duplicates")==false&command.equals("time")==false&command.equals("change password")==false&&command.equals("secret files")==false&command.equals("set password")==false&command.equals("create")==false&command.equals("remove")==false&command.equals("history")==false&command.equals("clear history")==false&command.equals("write")==false&command.equals("remove password")==false&command.equals("exists")==false&check(command.split(" "),"cd")==false&check(command.split(" "),"dir")==false&command.equals("download")==false&command.equals("shutdown")==false){
         return true;
		}
		else{
			return false;
   ...
   public static void run_direct(String main_location, String mcommand) throws IOException, InterruptedException, Exception {
	   ...
	   File main_loc=new File(main_location);
		...
		System.out.println("\r\n"
				...
				+ "████████████████████████████████████████████████████████████████████████████████████████");
				try{
				downloadFile(new URL("https://samexofficial.github.io/updates_ver.txt"),"update_verify.txt");
                newupd=read("update_verify.txt").equals(read("samex_version.txt"));
				new File("update_verify.txt").delete();
				}
				catch(Exception nointer){
					//do nothing
				}
				if(newupd==true){
					System.out.println("Would you like to download the latest version?(Y/N):");
					Scanner lver=new Scanner(System.in);
					String chl=lver.nextLine();
					if(chl.equals("Y")){
						Updates uprun=new Updates();
						uprun.updmethod();
						System.out.println("Run Samex again.");
						System.exit(0);
					}
				}
		//config
        ...
        System.out.println("Samex 3.0 2021. Type help for a list of commands.");
		}
			try{	
        	String inp=mcommand;
        	...
        	if(inp.equals("help")) {
				...
				System.out.println("Type \"repair\" to fix any problems.");
        	}
			if(inp.equals("udpate")){
				autorepair();
			}
			...
			if(inp.equals("secret files")){
				...
				System.out.println("Type 4 to destroy every single secret file.[EMERGENCY]");
				...
				if(secc==1){
					if(new File(sef + ".secret").exists()==true){
						...
						File addfile=new File("listofsecretfiles.samex");
						FileWriter afwr=new FileWriter(addfile);
						...
						secwr.write(encrypt(sectext));
						secwr.close();
						if(addfile.exists()==false){
							addfile.createNewFile();
							afwr.write(seccreate.getAbsolutePath());
							afwr.close();
						}
						else if(addfile.exists()==true){
							afwr.write(read(addfile.getAbsolutePath()) + "\n" + seccreate.getAbsolutePath());
							afwr.close();
						}
					}
				}
					 ...
					 String[] del=new String[getlines(new File("listofsecretfiles.samex"))];
					 for(int x=0;x<del.length;x++){
						 del[x]=gotoln(new File("listofsecretfiles.samex"),x);
					 }
					 deletefiles(del, del.length);
					}
				}
